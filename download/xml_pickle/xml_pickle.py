...
# Use mx.DateTime type, if possible
try:
  import mx.DateTime
  mxDateTime_type = type(mx.DateTime.localtime())
except:
  # no mx.DateTime pickles allowed
...
def _tag_completer(start_tag, thing, close_tag, level=0):
    ...
    if type(thing) == NoneType:
        ...
        close_tag = ''
    elif mxDateTime_type and type(thing) == mxDateTime_type:
        # This is not a minimal encoding, but it's compact while
        # still being editable. Args are in the same order
        # as mx.DateTime.DateTime() for convenience above.
        # (I avoided using strftime(), for portability reasons.)
        # Pickle seconds as a float to save full precision.
        start_tag = start_tag+'type="mxDateTime" '+ \
                    'value="%d %d %d %d %d %f" />\n' % \
                    (thing.year,thing.month,thing.day,\
                     thing.hour,thing.minute,thing.second)
        close_tag = ''
    else:
