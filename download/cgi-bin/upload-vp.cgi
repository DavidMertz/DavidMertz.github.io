...
dirUpload = "vp"                # directory for upload; will be created if doesn't exist
maxkb = 5000                    # maximum kilobytes to store before no more files accepted
link = "feedback.py"            # a page/url to link at the bottom of page after upload 
email = "uploads@gnosis.cx"     # where to email upload reports;
...
def form(posturl,files,kb,maxkb,button):
    ...
    print """<html>
<head>
<title>Voting Project File Upload</title>
<body>
...
if data.has_key("file.1"):  # we have uploads.
    ...
    if len(fnList):
        ...
        for x in range(0,len(fnList)):
            ...
            print "<LI>http://gnosis.cx/voting-project/%s (%.2f kb)" %\
                  (fnList[x],kbList[x] / 1024.0)
        print "</UL>"
        print "<P><HR>"
