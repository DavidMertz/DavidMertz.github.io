...
def ParseArgs(list):
    ...
    for item in list:
        ...
        if item[0] in '/-':                     # a switch!
            ...
            elif upper(item[1:6]) == 'PROXY':     # set proxy mode
                cfg_dict['proxy'] = upper(item[7:])
            elif upper(item[1:]) == 'REBUILD_DOCS':
...
def ParseCGI():
    ...
    if form.has_key('source'):
        val = form['source'].value
        if val[:7] == 'http://' or '/' not in val:
            cfg_dict['source'] = form['source'].value
        else:
            cfg_dict['source'] = 'txt2html.txt'
    else:
        # Third addition by Justin Swartz (inode@unix.za.net),
        # and finally '../rc/' was removed for continuity.
        # Seeing as it's included in the same directory as this
        # very file when extracted from txt2html.zip
        cfg_dict['source'] = 'txt2html.txt'
